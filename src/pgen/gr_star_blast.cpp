...
void MeshBlock::ProblemGenerator(ParameterInput *pin) {
  ...
  // Radius and polytropic index for star profile
    ...
    for (int j=jl; j<=ju; ++j) {
      ...
      for (int i=il; i<=iu; ++i) {
        ...
        if (rr < r_blast) {
          ...
          if (in_lobe) {
            const Real dens_reduction = 1e-3;
            const Real thermal_boost  = 10.0;
            rho0  *= dens_reduction;
            pgas0 *= thermal_boost;
          }
        }
        ...
        // Add Kinetic
        bool in_lobe = (rr < r_blast) &&
            ((theta_dir >= dir_angle_min && theta_dir <= dir_angle_max) ||
             (theta_dir >= M_PI - dir_angle_max && theta_dir <= M_PI - dir_angle_min));
        Real sign    = (theta_dir <= M_PI/2 ? 1.0 : -1.0);
        Real p_local = in_lobe ? dir_blast_factor : 0.0;
        Real ut      = std::sqrt(1.0 + p_local*p_local);
        Real ux      = 0.0;
        Real uy      = 0.0;
        Real uz      = sign * p_local;
